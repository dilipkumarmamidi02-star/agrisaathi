...
export default function CommunityForum() {
  ...
  if (view === 'detail' && active) {
    return (
      <div>
        <button onClick={() => { setView('list'); setActive(null); }} className="flex items-center gap-2 text-sm text-lt-success mb-3">
        ...
        <Card className="mb-4"><CardContent className="pt-4 space-y-2">
          ...
          <p className="text-xs text-lt-text-muted">{active.author_name} · {new Date(active.created_date).toLocaleDateString('en-IN')}</p>
          <p className="text-sm text-lt-text">{active.body}</p>
          <Button size="sm" variant="ghost" onClick={() => upvote(active)}><ThumbsUp className="h-3 w-3 mr-1" />{active.upvotes || 0}</Button>
        </CardContent></Card>

        <h3 className="text-sm font-semibold text-lt-text-secondary mb-2">{t('replies')} ({(active.replies || []).length})</h3>
        <div className="space-y-2 mb-3">
          {(active.replies || []).map((r, i) => (
            <Card key={i}><CardContent className="pt-3">
              <p className="text-xs text-lt-text-muted mb-0.5">{r.author_name} · {new Date(r.ts).toLocaleDateString('en-IN')}</p>
              <p className="text-sm text-lt-text">{r.body}</p>
            </CardContent></Card>
          ))}
          {(active.replies || []).length === 0 && <p className="text-xs text-lt-text-muted">{t('noReplies')}</p>}
        </div>
  ...
  if (view === 'new') {
    return (
      <div>
        <button onClick={() => setView('list')} className="flex items-center gap-2 text-sm text-lt-success mb-3">
          <ArrowLeft className="h-4 w-4" /> {t('backToForum')}
        </button>
  ...
  return (
    <div>
      ...
      <p className="text-xs text-lt-text-secondary mb-3">{t('forumIntro')}</p>
      ...
      {posts.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">{t('noPosts')}</CardContent></Card>
      ) : (
        <div className="space-y-2">
          {posts.map((p) => (
            <Card key={p.id} className="cursor-pointer hover:border-lt-success/40" onClick={() => { setActive(p); setView('detail'); }}>
              <CardContent className="pt-3 pb-3">
                <div className="flex items-start justify-between gap-2">
                  <h3 className="font-semibold text-sm leading-tight">{p.title}</h3>
                  <Badge variant="secondary">{p.category}</Badge>
                </div>
                <p className="text-xs text-lt-text-secondary mt-1 line-clamp-2">{p.body}</p>
                <div className="flex items-center gap-3 mt-2 text-xs text-lt-text-muted">
