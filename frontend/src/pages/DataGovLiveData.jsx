...
function ResourceCard({
  ...
  let badgeClass =
    'bg-lt-bg text-lt-text border-lt-border';

  if (status === LIVE_STATUS) {
    badgeClass =
      'bg-lt-success/10 text-lt-success border-lt-primary/20';
  }
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div className="min-w-0">
          <h3 className="font-semibold text-lt-text break-words">
            {title}
          </h3>

          <p className="mt-1 text-xs text-lt-text-secondary break-all">
            {key}
          </p>
        </div>
      ...
      {status === ERROR_STATUS && (
        <div className="mt-3">
          <span
            ...
            className="inline-flex rounded-lg border border-red-200 bg-lt-card px-3 py-1.5 text-xs font-medium text-red-400 hover:bg-red-500/10"
          >
            Retry
          </span>
        </div>
...
function ResourceTable({ records }) {
  ...
  if (!records.length) {
    return (
      <div className="rounded-xl border border-lt-border bg-lt-card p-8 text-center text-lt-text-secondary">
  ...
  return (
    <div className="overflow-auto rounded-xl border border-lt-border bg-lt-card">
      <table className="min-w-full text-sm">
        <thead className="bg-lt-bg">
          <tr>
            {columns.map((column) => (
              <th
                key={column}
                className="whitespace-nowrap border-b px-4 py-3 text-left font-semibold text-lt-text"
              >
                {column}
              </th>
            ))}
        ...
        <tbody>
          {records.map((record, index) => (
            <tr
              ...
              className="border-b last:border-b-0 hover:bg-lt-bg"
            >
              {columns.map((column) => (
                <td
                  key={column}
                  className="max-w-[360px] px-4 py-3 align-top text-lt-text"
                >
...
export default function DataGovLiveData() {
  ...
  if (loadingResources) {
    return (
      <div className="p-6">
        <div className="rounded-xl border bg-lt-card p-8 text-center">
  ...
  return (
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-4">
        <div className="rounded-xl border bg-lt-card p-4">
        ...
        <div className="rounded-xl border bg-lt-card p-4">
          ...
          <div className="mt-1 text-2xl font-bold text-lt-success">
            {summary.live}
          </div>
        </div>
        ...
        <section className="space-y-3">
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className="rounded-lg border px-3 py-1.5 text-xs font-medium hover:bg-lt-bg"
            >
              Refresh registry
            </button>
          </div>
        ...
        <section>
          {selectedResource ? (
            <div className="space-y-4">
              <div className="rounded-xl border bg-lt-card p-5">
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 break-all text-xs text-lt-text-secondary">
                      {selectedKey}
                    </p>
                  </div>
              ...
              {loadingKey === selectedKey && (
                <div className="rounded-xl border bg-lt-card p-8 text-center text-lt-text-secondary">
                ...
                selectedResult?.status === ERROR_STATUS && (
                  <div className="rounded-xl border border-red-200 bg-red-500/10 p-6">
                    ...
                    <button
                      type="button"
                      onClick={() => loadResource(selectedKey)}
                      className="mt-4 rounded-lg border border-red-300 bg-lt-card px-4 py-2 text-sm font-medium text-red-400 hover:bg-red-100"
                    >
                      Retry resource
                    </button>
                  </div>
