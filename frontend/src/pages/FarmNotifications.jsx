...
export default function FarmNotifications() {
  ...
  return (
    <div>
      ...
      {loading ? (
        <p className="text-sm text-lt-text-muted text-center py-8">Loading reminders…</p>
      ) : reminders.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">No reminders set yet.</CardContent></Card>
      ) : (
              ...
              <CardContent className="pt-3 pb-3 flex items-center justify-between">
                <button onClick={() => !b.payload.done && markDone(b.payload.title, b.payload.due_date)} className="flex items-center gap-2 text-left flex-1">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-lt-text-muted">{new Date(b.payload.due_date).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                  </div>
                </button>
              </CardContent>
            </Card>
