...
export default function GovernmentSchemes() {
  ...
  const statusStyle = (s) =>
    s === 'likely_eligible' ? 'bg-lt-success/20 text-lt-success' :
  ...
  return (
    <div>
      ...
      {loading ? (
        ...
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">No schemes found for this state yet.</CardContent></Card>
      ) : (
          ...
          {schemes.map((s) => {
            ...
            return (
              <Card key={s.id}><CardContent className="pt-4 space-y-2">
                ...
                {s.ministry && <p className="text-xs text-lt-text-secondary">{s.ministry}</p>}
                <p className="text-sm text-lt-text">{s.benefit_summary}</p>
                <p className="text-xs text-lt-text-secondary"><span className="font-medium">Eligibility:</span> {s.eligibility_summary}</p>

                {result?.reason && <div className="bg-lt-bg rounded-lg p-2 text-xs text-lt-text-secondary">{result.reason}</div>}
                {result?.next_steps && <div className="bg-lt-success/10 rounded-lg p-2 text-xs text-lt-success">{result.next_steps}</div>}

                {isOpen && questions.length > 0 && (
                  <div className="space-y-2 bg-lt-bg rounded-lg p-3">
                    {questions.map((q) => (
                      <div key={q.field} className="flex items-center justify-between gap-2">
                        <span className="text-xs text-lt-text flex-1">{q.label}</span>
                          ...
                          {[['Yes', true], ['No', false]].map(([label, val]) => (
                            <button
                              ...
                              className={`px-2 py-1 rounded text-xs border ${
                                answers[s.id]?.[q.field] === val ? 'bg-lt-primary text-white border-lt-primary' : 'bg-lt-card text-lt-text-secondary border-lt-border'
                              }`}
                            >
                              {label}
                            </button>
