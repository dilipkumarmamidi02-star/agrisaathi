  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <Card><CardContent className="pt-3"><p className="text-xs text-lt-text-secondary">{t('estValue')}</p><p className="text-lg font-bold text-lt-success">₹{totalValue.toLocaleString('en-IN')}</p></CardContent></Card>
      </div>

      {records.length > 0 && (
        <Card className="mb-4"><CardContent className="pt-4">
          <h3 className="text-sm font-semibold text-lt-text mb-2 flex items-center gap-1.5"><Wheat className="h-4 w-4 text-lt-primary" />{t('yieldByCropSeason')}</h3>
          {(() => {
            const byKey = {};
            records.forEach((r) => {
      ...
      {plots.length > 0 && (
        <Card className="mb-4"><CardContent className="pt-4">
          <h3 className="text-sm font-semibold text-lt-text mb-2 flex items-center gap-1.5"><TrendingUp className="h-4 w-4 text-lt-primary" />{t('yieldTrend')}</h3>
          {plots.length > 1 && (
          ...
          {trendData.length >= 2 ? (
            ...
            <p className="text-xs text-lt-text-muted">{t('yieldTrendEmpty')}</p>
          )}
        </CardContent></Card>
      )}
          ...
          {records.map((r) => (
            <Card key={r.id}><CardContent className="pt-3 pb-3 flex items-center justify-between gap-2">
              <div className="min-w-0">
                ...
                <p className="text-xs text-lt-text-secondary">{r.harvest_date} · {r.quantity || 0} {r.quantity_unit}{r.sale_price_per_unit ? ` · ₹${r.sale_price_per_unit}/unit` : ''}</p>
              </div>
              <div className="flex items-center gap-1.5 shrink-0">
                {r.quality_grade && <Badge className="bg-lt-success/20 text-lt-success">{r.quality_grade}</Badge>}
