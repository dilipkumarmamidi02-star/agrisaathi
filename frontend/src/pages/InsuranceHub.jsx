...
const CLAIM_STATUS = {
  ...
  approved: { label: 'Approved', color: 'bg-lt-success/20 text-lt-success' },
    ...
    <div>
      ...
      <div className="grid grid-cols-2 gap-2 mb-4">
        <Card className="bg-lt-success/10 border-lt-primary/10"><CardContent className="pt-3 text-center">
          <div className="text-2xl font-bold text-lt-success">{active.length}</div>
          <div className="text-[11px] text-lt-text-secondary">{t('activePolicies')}</div>
        </CardContent></Card>
        <Card className="bg-amber-500/10 border-amber-100"><CardContent className="pt-3 text-center">
          ...
          <div className="text-[11px] text-lt-text-secondary">{t('filedClaims')}</div>
        </CardContent></Card>
      </div>

      <div className="space-y-2 mb-4">
        {policies.length === 0 ? (
          <p className="text-sm text-lt-text-muted">{t('noPolicies')}</p>
        ) : policies.map((p) => {
      ...
      {showAdd ? (
        ...
        <Button onClick={() => setShowAdd(true)} variant="outline" className="w-full border-lt-success/40 text-lt-success"><Plus className="h-4 w-4 mr-1" />{t('addPolicy')}</Button>
      )}
