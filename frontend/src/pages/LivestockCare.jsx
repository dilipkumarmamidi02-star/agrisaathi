...
export default function LivestockCare() {
  ...
  const careColor = (c) => ({ feed: 'bg-blue-100 text-cyan-400', health: 'bg-rose-100 text-rose-700', milestone: 'bg-purple-100 text-purple-700', vaccination: 'bg-teal-100 text-teal-700' }[c] || 'bg-lt-bg');
  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-3 gap-2 mb-4">
        <Card className="bg-cyan-500/10 border-blue-100"><CardContent className="pt-3 text-center"><div className="text-xl font-bold text-cyan-400">{logs.filter((l) => l.care_type === 'feed').length}</div><div className="text-[10px] text-lt-text-secondary">{t('feed')}</div></CardContent></Card>
        <Card className="bg-rose-50 border-rose-100"><CardContent className="pt-3 text-center"><div className="text-xl font-bold text-rose-700">{logs.filter((l) => l.care_type === 'health').length}</div><div className="text-[10px] text-lt-text-secondary">{t('health')}</div></CardContent></Card>
        <Card className="bg-teal-50 border-teal-100"><CardContent className="pt-3 text-center"><div className="text-xl font-bold text-teal-700">{logs.filter((l) => l.care_type === 'vaccination').length}</div><div className="text-[10px] text-lt-text-secondary">{t('vaccination')}</div></CardContent></Card>
      </div>

      <h3 className="text-sm font-semibold text-lt-text mb-2">{t('pending')}</h3>
      <div className="space-y-2 mb-4">
        {pending.length === 0 ? <p className="text-sm text-lt-text-muted">No pending care items.</p> : pending.map((l) => (
          <Card key={l.id}><CardContent className="pt-3">
            <div className="flex items-start justify-between gap-2">
              <div className="min-w-0">
                <div className="flex items-center gap-1.5 flex-wrap">
                  <Badge className={careColor(l.care_type)}>{t(l.care_type)}</Badge>
                  <span className="text-xs text-lt-text-muted">{l.animal_type}</span>
                </div>
                <p className="text-sm font-medium mt-1">{l.title}</p>
                {l.scheduled_date && <p className="text-xs text-lt-text-muted flex items-center gap-1"><Calendar className="h-3 w-3" />{l.scheduled_date}</p>}
                {l.notes && <p className="text-xs text-lt-text-secondary mt-0.5">{l.notes}</p>}
              </div>
      ...
      <h3 className="text-sm font-semibold text-lt-text-secondary mb-2">{t('done')}</h3>
      <div className="space-y-2 mb-4">
        {done.map((l) => (
          <Card key={l.id} className="opacity-60"><CardContent className="pt-3">
            <div className="flex items-center gap-1.5"><Badge className="bg-lt-bg text-lt-text-secondary">{t(l.care_type)}</Badge><span className="text-xs text-lt-text-muted">{l.animal_type}</span></div>
      ...
      {showAdd ? (
        ...
        <Button onClick={() => setShowAdd(true)} variant="outline" className="w-full border-lt-success/40 text-lt-success"><Plus className="h-4 w-4 mr-1" />{t('addEntry')}</Button>
      )}
    </div>
  );
}
