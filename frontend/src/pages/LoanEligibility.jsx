...
Known required documents: ${(loan.required_documents || []).join(', ')}`,
  ...
  const statusStyle = (s) =>
    s === 'eligible' ? 'bg-lt-success/20 text-lt-success' :
          ...
          {loans.map((l) => {
            ...
            return (
              <Card key={l.id}><CardContent className="pt-4 space-y-2">
                ...
                {l.provider && <p className="text-xs text-lt-text-secondary">{l.provider}</p>}
                ...
                {l.purpose && <p className="text-sm text-lt-text">{l.purpose}</p>}
                {l.eligibility_summary && <p className="text-xs text-lt-text-secondary"><span className="font-medium">Eligibility:</span> {l.eligibility_summary}</p>}
                {r?.reason && <div className="bg-lt-bg rounded-lg p-2 text-xs text-lt-text-secondary">{r.reason}</div>}
                ...
                {r?.next_steps && <div className="bg-lt-success/10 rounded-lg p-2 text-xs text-lt-success">{r.next_steps}</div>}
