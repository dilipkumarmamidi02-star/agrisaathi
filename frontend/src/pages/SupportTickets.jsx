...
export default function SupportTickets() {
  ...
  return (
    <div>
      ...
      <DynamicScenePhoto fallbackQuery="customer support helpdesk" openCount={openCount} resolvedCount={resolvedCount} />
      ...
      {loading ? (
        <p className="text-sm text-lt-text-muted text-center py-8">Loading tickets…</p>
      ) : tickets.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">No tickets yet. Raise one above if you run into a problem.</CardContent></Card>
      ) : (
          ...
          {tickets.map((b) => (
            <Card key={b.index}>
              <CardContent className="pt-3 pb-3">
                ...
                <p className="text-[11px] text-lt-text-muted mt-0.5">{b.payload?.category} · {new Date(b.timestamp).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}</p>
                <p className="text-xs text-lt-text-secondary mt-1">{b.payload?.description}</p>
              </CardContent>
            </Card>
          ))}
        </div>
