  ...
  return (
    <div>
      ...
      <div className="flex gap-1.5 mb-4 overflow-x-auto pb-1">
        {CATEGORIES.map((c) => (
          <button key={c.value} onClick={() => setFilter(c.value)} className={`px-3 py-1 rounded-full text-xs whitespace-nowrap ${filter === c.value ? 'bg-lt-primary text-white' : 'bg-lt-bg text-lt-text-secondary'}`}>
            {t(c.label)}
          </button>
        ))}
      </div>
      ...
      {list.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">{t('noResources')}</CardContent></Card>
      ) : (
            ...
            return (
              <Card key={r.id}><CardContent className="pt-3 pb-3">
                <div className="flex items-start gap-3">
                  <span className={`flex h-10 w-10 items-center justify-center rounded-xl shrink-0 ${r.type === 'video' ? 'bg-red-500/10 text-red-600' : 'bg-cyan-500/10 text-blue-600'}`}>
                    <Icon className="h-5 w-5" />
                  </span>
                  <div className="min-w-0 flex-1">
                    <h3 className="font-semibold text-sm leading-tight">{r.title}</h3>
                    {r.summary && <p className="text-xs text-lt-text-secondary mt-0.5 line-clamp-2">{r.summary}</p>}
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                      <Badge variant="secondary">{t(r.category)}</Badge>
                      {r.difficulty && <Badge className={r.difficulty === 'beginner' ? 'bg-lt-success/20 text-lt-success' : r.difficulty === 'intermediate' ? 'bg-amber-100 text-amber-400' : 'bg-red-100 text-red-400'}>{t(r.difficulty)}</Badge>}
                      {r.duration_minutes != null && <span className="text-xs text-lt-text-muted flex items-center gap-0.5"><Clock className="h-3 w-3" />{r.duration_minutes}m</span>}
                      {r.language && r.language !== 'en' && <Badge variant="outline">{r.language}</Badge>}
                    </div>
                  </div>
                  {r.url && (
                    <a href={r.url} target="_blank" rel="noreferrer" className="shrink-0 flex h-8 w-8 items-center justify-center rounded-full bg-lt-success/10 text-lt-primary">
