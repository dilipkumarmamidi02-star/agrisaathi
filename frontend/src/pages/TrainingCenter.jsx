...
export default function TrainingCenter() {
  ...
  return (
    <div>
      ...
      <p className="text-xs text-text-secondary mb-3">{t('trainingIntro')}</p>
      ...
      {list.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-text-muted">{t('noResources')}</CardContent></Card>
      ) : (
              ...
              <Card key={r.id}><CardContent className="pt-3 pb-3">
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {r.summary && <p className="text-xs text-text-secondary mt-0.5 line-clamp-2">{r.summary}</p>}
                    <div className="flex flex-wrap items-center gap-1.5 mt-1.5">
                      <Badge variant="secondary">{t(r.category)}</Badge>
                      {r.difficulty && <Badge className={r.difficulty === 'beginner' ? 'bg-mint/20 text-mint' : r.difficulty === 'intermediate' ? 'bg-amber-100 text-amber-400' : 'bg-red-100 text-red-400'}>{t(r.difficulty)}</Badge>}
                      {r.duration_minutes != null && <span className="text-xs text-text-muted flex items-center gap-0.5"><Clock className="h-3 w-3" />{r.duration_minutes}m</span>}
