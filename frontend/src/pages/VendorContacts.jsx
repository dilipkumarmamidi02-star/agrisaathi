...
export default function VendorContacts() {
  ...
  return (
    <div>
      ...
      {loading ? (
        <p className="text-sm text-lt-text-muted text-center py-8">Loading vendors…</p>
      ) : vendors.length === 0 ? (
        <Card><CardContent className="pt-6 text-center text-sm text-lt-text-muted">No vendors saved yet. Add one above.</CardContent></Card>
      ) : (
            ...
            <Card key={b.payload.name}>
              <CardContent className="pt-3 pb-3 flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button onClick={() => removeVendor(b.payload.name)} className="p-2 text-lt-text-muted hover:text-red-500" aria-label={`Remove ${b.payload.name}`}>
